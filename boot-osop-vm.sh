...
net_id=`neutron net-create osop-net | grep "\ id\ " | awk '{print $4}'`
subnet_id=`neutron subnet-create $net_id "192.168.0.0/24"`
...
nova boot --image $image_id --flavor $flavor_id --nic net-id=$net_id --poll osop-vm

# Cleanup resources
nova_id=`nova list | grep "osop-vm" | awk '{print $2}'`
nova delete $nova_id
